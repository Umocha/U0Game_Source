...
#include "U0GameplayAbility_RangedWeapon.h"
#include "U0RangedWeaponInstance.h"
#include "U0Game/AbilitySystem/U0GameplayAbilityTargetData_SingleTarget.h"
#include "U0Game/AbilitySystem/U0AbilitySystemComponent.h"
#include "CollisionQueryParams.h"
#include "U0Game/Physics/U0CollisionChannels.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(U0GameplayAbility_RangedWeapon)

UU0GameplayAbility_RangedWeapon::UU0GameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UU0GameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	// ActionInfo는 AbilitySet에서 GiveAbility() 호출로 결정된다
	// - UGameplayAbility::OnGiveAbility()에서 SetCurrentActorInfo()에서 설정된다
	// - AbilitySystemComponent::GiveAbility()에서 OnGiveAbility() 호출한다
	// - U0AbilitySet::GiveToAbilitySystem()에서 GiveAbility()를 호출한다
	...
	// *** 여기서 Lyra는 샷건 처리와 같은 탄착 처리등등함, 우리는 생략하고 권총으로 진행하였다 (아래의 로직은 간단버전이다)
	...
	if (FoundHits.Num() > 0)
	{
		// Cartridge란 일반 권총의 경우, 탄약에 하나의 총알이 들어있지만, 샷건의 경우 탄약에 여러개의 총알이 있고, **탄약을 카트리지로 생각** 하면 된다
		const int32 CartridgeID = FMath::Rand();
		for (const FHitResult& FoundHit : FoundHits)
		{
			// AbilityTargetData에 SingleTargetHit 정보를 담는다
			// - 참고로 TargetData.Add()의 경우, SharedPtr에 넣기 때문에 여기서 new는 크게 신경 안써도된다
			FU0GameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FU0GameplayAbilityTargetData_SingleTargetHit();
...
void UU0GameplayAbility_RangedWeapon::PerformLocalTargeting(TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());

	UU0RangedWeaponInstance* WeaponData = GetWeaponInstance();
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, EU0AbilityTargetingSource::CameraTowardsFocus);
...
#if 0
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, 60.0f, 0, DebugThickness);
		}
#endif
...
FTransform UU0GameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, EU0AbilityTargetingSource Source)
{
	check(SourcePawn);
	check(Source == EU0AbilityTargetingSource::CameraTowardsFocus);
...
FVector UU0GameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
void UU0GameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, TArray<FHitResult>& OutHits)
{
	UU0RangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
...
FHitResult UU0GameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, TArray<FHitResult>& OutHits) const
{
	FHitResult Impact;

	// 우선 SweepTadius 없이 한번 Trace 진행한다 (SweepTrace는 무겁기 때문)
	// - FindFirstPawnHitResult()를 여러번 Trace 진행을 막기 위해, OutHits를 확인해서 APawn 충돌 정보가 있으면 더이상 Trace하지 않는다
	...
	if (FindFirstPawnHitResult(OutHits) == INDEX_NONE)
	{
		// 만약 SweepRadius가 0보다 크면, 0일때 대비 충돌 가능성이 커지므로 한번 더 Trace 진행
		if (SweepRadius > 0.0f)
		{
			...
			if (SweepHits.IsValidIndex(FirstPawnIdx))
			{
				...
				for (int32 Idx = 0; Idx < FirstPawnIdx; ++Idx)
				{
					...
					// OutHits에 있다면... SweepHits를 OutHits로 업데이트 하지 않는다 (이미 충돌했던 정보가 있으니까?) (early-out용)
...
FHitResult UU0GameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, TArray<FHitResult>& OutHits) const
{
...
void UU0GameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel UU0GameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return U0_TraceChannel_Weapon;
}

void UU0GameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilitySystemComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		{
			// CommitAbility가 실패했으면, EndAbility BP Node 호출한다
			K2_EndAbility();
		}
	}
}

UU0RangedWeaponInstance* UU0GameplayAbility_RangedWeapon::GetWeaponInstance()
{
	return Cast<UU0RangedWeaponInstance>(GetAssociatedEquipment());
}
