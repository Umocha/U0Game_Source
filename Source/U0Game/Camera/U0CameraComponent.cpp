...
#include UE_INLINE_GENERATED_CPP_BY_NAME(U0CameraComponent)

UU0CameraComponent::UU0CameraComponent(const FObjectInitializer& ObjectInitializer)
...
void UU0CameraComponent::UpdateCameraModes()
{
	...
	{
		if (TSubclassOf<UU0CameraMode> CameraMode = DetermineCameraModeDelegate.Execute())
		{
...
void UU0CameraComponent::OnRegister()
{
	Super::OnRegister();

	if (!CameraModeStack)
	{
		// 초기화 (BeginPlay와 같은)가 딱히 필요없는 객체라서 NewObject로 할당
		CameraModeStack = NewObject<UU0CameraModeStack>(this);
	}
}

void UU0CameraComponent::GetCameraView(float DeltaTime, FMinimalViewInfo& DesiredView)
{
	check(CameraModeStack);
	...
	FU0CameraModeView CameraModeView;
	...
	if (APawn* TargetPawn = Cast<APawn>(GetTargetActor()))
	{
		if (APlayerController* PC = TargetPawn->GetController<APlayerController>())
		{
			...
			//     - 해당 함수는 PC가 Possess하고 있는 Pawn의 RootComponent에 ControlRotation을 반영한다
	...
	*/
	...
	// - CameraComponenet의 변화 사항을 반영해서 최종 렌더링까지 반영하게 됨
